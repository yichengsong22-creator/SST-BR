...
class BaselineResidualRegressor:
    ...
    def fit(self, features: np.ndarray, labels: np.ndarray, subject_ids: np.ndarray) -> "BaselineResidualRegressor":
        ...
        # Baseline branch: predict subject-level HR from the long-time
        # radar-only temporal context.
        ...
        # The residual target follows the manuscript's baseline–residual
        # decomposition and retains window-specific variation.
    ...
    def combine_hr(baseline: float, residual: float) -> float:
        ...
        # Recombine both branches to obtain the window-level HR estimate.
...
def train_and_predict(config: dict, features: np.ndarray, metadata: pd.DataFrame, labels5: pd.DataFrame, labels10: pd.DataFrame, feature_names: list[str]) -> tuple[pd.DataFrame, pd.DataFrame, dict, dict]:
    ...
    for fold_index in range(1, 5):
        ...
        # Train the residual branch on local deviations from the training-only
        ...
        # Recombine baseline and residual predictions before the verified HR
        # bounds are applied.
